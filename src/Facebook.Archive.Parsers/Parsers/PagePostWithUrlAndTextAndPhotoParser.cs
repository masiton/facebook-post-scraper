...
{
    public class PagePostWithUrlAndTextAndPhotoParser : GenericParser, IParser<PagePostWithUrlAndTextAndPhoto>
    {
        public string ParserName => nameof(PagePostWithUrlParser);

        public int ParserVersion => 1;

        public PagePostWithUrlAndTextAndPhoto Parse(HtmlNode node)
        {
            var post = new PagePostWithUrlAndTextAndPhoto
            {
                Html = node.InnerHtml,
